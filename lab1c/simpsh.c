...
void setStartTimes(){
  ...
  if(ret < 0)
    fprintf(stderr, "unable to get resource usage. error: %d, message: %s", errno, strerror(errno));
  user_start = usage.ru_utime;
  system_start = usage.ru_stime;
  return;
}

void setEndTimes(char* opt){
  ret = getrusage(who, &usage);
  if(ret < 0)
    fprintf(stderr, "unable to get resource usage. error: %d, message: %s", errno, strerror(errno));
  ...
  struct timeval diff_user;
  struct timeval diff_system;

  diff_user.tv_sec = user_end.tv_sec - user_start.tv_sec;
  diff_user.tv_usec = user_end.tv_usec - user_start.tv_usec;
  diff_system.tv_sec = system_end.tv_sec - system_start.tv_sec;
  diff_system.tv_usec = system_end.tv_usec - system_start.tv_usec;
  fprintf(stdout, "%s user: %ld.%06ld s, system: %ld.%06ld s \n", opt, diff_user.tv_sec, diff_user.tv_usec, diff_system.tv_sec, diff_system.tv_usec);
  return;
...
int main(int argc, char** argv){
    ...
    struct timeval child_user_start;
    struct timeval child_system_start; 
    ...
    child_user_start = usage.ru_utime;
    child_system_start = usage.ru_stime;
    ...
    if(children && profile){ //if any child processes were started
      who = RUSAGE_CHILDREN;
      ret = getrusage(who, &usage);
      if(ret < 0)
	fprintf(stderr, "unable to get resource usage. error: %d, message: %s", errno, strerror(errno));
      
      struct timeval child_user_end = usage.ru_utime;
      struct timeval child_system_end = usage.ru_stime;
      struct timeval diff_user;
      struct timeval diff_system;
  
      diff_user.tv_sec = child_user_end.tv_sec - child_user_start.tv_sec;
      diff_user.tv_usec = child_user_end.tv_usec - child_user_start.tv_usec;
      diff_system.tv_sec = child_system_end.tv_sec - child_system_start.tv_sec;
      diff_system.tv_usec = child_system_end.tv_usec - child_system_start.tv_usec;
      if(!(diff_user.tv_sec == 0 && diff_user.tv_usec == 0 && diff_system.tv_sec == 0 && diff_system.tv_usec == 0))
	fprintf(stdout, "child processes user: %ld.%06ld s, system: %ld.%06ld s \n", diff_user.tv_sec, diff_user.tv_usec, diff_system.tv_sec, diff_system.tv_usec);
