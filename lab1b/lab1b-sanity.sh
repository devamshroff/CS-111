...
if [ $T9RC == 0 ] && [ -e test9err.txt ] && [ -e test9out.txt ] \
       && [ ! -s test9err.txt ]  && [ ! -s c9out.txt ] && cmp -s test9outstd.txt test9out.txt \
...
# Test case 18 --wait, part 3: if we don't close 4 in parent process, cat will
# hang there forever.
echo ""
echo "--->test case 18:"
...
./simpsh --creat --excl --rdwr test20io.txt --command 0 0 0 time -p sleep 1 \
  --wait --rdonly a8.txt --command 1 0 0 cat >c20out.txt 2>c20err.txt
if [ $? == 0 ] && [ -e test20io.txt ] && grep -q "real 1." test20io.txt \
