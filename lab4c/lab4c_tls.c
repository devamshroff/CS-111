...
void write_message(char* message){
    if(SSL_write(ssl, message, strlen(message)) < 0){
        fprintf(stderr, "ssl write error:%d message: %s\n", errno, strerror(errno));
        exit(2);
    }
}
double getTemp(){
    ...
    R = R0*R;
    float x = log(R/R0);
    float temperature = 1.0/(x/B+1/298.15)-273.15;
    if(temperature < 0){
...
int main(int argc, char ** argv){
    ...
    if(SSL_library_init() < 0){
        fprintf(stderr, "ssl initialization error. error: %d, message: %s\n", errno, strerror(errno));
        exit(2);
    }
    SSL_CTX *ssl_ctx = SSL_CTX_new(TLSv1_client_method());
    if(ssl_ctx == NULL){
       fprintf(stderr, "ctx error. error: %d, message: %s\n", errno, strerror(errno));
       exit(2);
    }
    ssl = SSL_new(ssl_ctx);
    if(SSL_set_fd(ssl, socket_fd)<0) {
       fprintf(stderr, "ssl_ctx error: %d, message: %s\n", errno, strerror(errno));
       exit(2);
    }
    if(SSL_connect(ssl) != 1){
        fprintf(stderr, "ssl connection error. error: %d, message: %s\n", errno, strerror(errno));
        exit(2);
    }
